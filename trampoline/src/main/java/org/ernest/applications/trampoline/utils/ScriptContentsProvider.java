...
public class ScriptContentsProvider {
    ...
    public static String getGradleWindows(String microServiceName, String pomLocation){
        return "SET server.port=#port&& SET endpoints.shutdown.enabled=true&& SET management.security.enabled=false #vmArguments&& cd "+pomLocation+" && gradlew.bat " + microServiceName + ":bootRun ";
    }
    ...
    public static String getGradleUnix(String microServiceName, String pomLocation){
        return "export SERVER_PORT=$1; export ENDPOINTS_SHUTDOWN_ENABLED=true; export MANAGEMENT_SECURITY_ENABLED=false $2; cd " + pomLocation + "; ./gradlew " + microServiceName + ":bootRun";
    }
}
